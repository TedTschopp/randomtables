...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    function expand_token(string, local_variable_store) {
        ...
        if (match_1 = /^\d*d\d+/.exec(string))
        ...
        } else if (d = gen_data[string])
            return expand_tokens(select_from(d), local_variable_store);
        else if (match_1 = /^alt (.+) def (.+)/.exec(string))
            /* alt xxx def xxx */
            return (d = gen_data[match_1[1]]) ?
                expand_tokens(select_from(d), local_variable_store) :
                (d = gen_data[match_1[2]]) ?
                    expand_tokens(select_from(d), local_variable_store) :
                    match_1[2];
            ...
            return local_variable_store["var"][match_1[1]] ||
                set_var(match_1[1], match_1[2], local_variable_store);
